...
- 2026-08-31 — `max_batch_size=50` is a protocol constant in the API layer. SPEC §6.3 describes batch semantics but does not specify a maximum size; 50 is a pragmatic bound that prevents a single request from monopolising the semaphore pool.
...
- 2026-08-31 (session 2 cont., compactions: 5) — vm-batches (§6.3) completed, P3 now done. Landed: store migration v5 (vm_batches + vm_batch_members, UNIQUE partial index for idempotency); `internal/store/batches.go` (CreateVMBatch: atomic_reservation and best_effort modes, AT-012 in-tx cumulative admission, AT-015 idempotent replay, ErrIdempotencyConflict, GetVMBatch); `internal/runtime/batch.go` (Manager.CreateBatch: AT-001 availability first, per-member validation + defaults, batchCoord for AT-014 stop_successful, async launch with batch-op finalization); `internal/api/batches.go` (POST /vm-batches, GET /vm-batches/{id}, writeBatchError, max_batch_size=50); api.go routes + limits updated, 10 API tests + 8 store tests. `scripts/check` fully green. 2 commits. Branch not pushed — merge is Doctor Biz's call. Next: P4 (declarative runs + reports).
