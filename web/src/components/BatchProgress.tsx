...
function MemberRow({ member }: { member: BatchMember }) {
  ...
  return (
    <li className={`member-row member-${outcome}`} data-testid={`member-row-${member.position}`}>
      {/* The member name came from an operator's paste and rode through the
          daemon unchanged; it is data, never an instruction to the terminal. */}
      <span className="member-name">{member.name ? neutralize(member.name) : `member ${member.position}`}</span>
      ...
      {cause && (
        <span className="member-cause">
          <code>{neutralize(cause.cause)}</code> {neutralize(cause.message)}
        </span>
      )}
    </li>
  )
