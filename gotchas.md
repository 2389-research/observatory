...
- **No process ceremony with Harper.** Don't announce skill usage, workflow compliance, or plans-before-acting. They interrupted mid-task to stop it ("please stop"). Keep the discipline (verify, be honest about what ran), skip the narration — the first visible action should be real work.
...
- **Manager shutdown needs separate worker cancellation and recovery writes.** The current `wg.Wait()` before `cancel()` preserves store writes but can hang on initial/batch launches waiting for cancellation. Simply reversing the calls loses final state writes. Kata `3dnv` tracks bounded shutdown; synchronous start already has `launchBudget` and `recoveryContext`, while initial/batch launches do not.
...
whether the VMM is gone. Until `12e3cd6` that compared only the entry and
`/proc/<pid>/stat` field 22, so a stand-in process a test killed without a
matching `cmd.Wait()` became a zombie and read alive. Four proof tests failed
that way and read exactly like the bug they exist to catch. PIDAlive now reads
the state field too and calls `Z` and `X` dead, but two checks still call a
zombie alive: `runnerAlive`'s last fallback, a bare `os.Stat("/proc/<pid>")`
reached only when the manifest has no start time and the argv is unreadable,
and `kill(pid, 0)`. In production the VMM is daemonized under privd and
reparented to init, which reaps it — so a helper that spawns a stand-in owes it
a reaper goroutine, and the cleanup has to wait on that goroutine before the
test's temp dir goes away.
...
Do not ask again for the design decision they just delegated.
...
## A runner outlives its VMM by one watch tick (kvcn)

`vmobs-runner` checks its VMM once a second. When the VMM dies it appends
`vm.vmm_exited` and writes `runner-state.json` twice (`vmm_exited`, then
`finalized`), each through a temp file, fsync and rename in
`<StateDir>/vms/<id>/`. Killing the VMM does not stop those writes. Go's
`t.TempDir()` cleanup is one `os.RemoveAll` with no retry off Windows, and a
temp file created after its readdir fails the last `unlinkat` with "directory
not empty": 1 run in 150 of `TestReconcileAdoptsHealthyVM` on aibox03. A
jailer test that launches a real runner goes through `launchTestVM`, whose
cleanup kills the stand-in VMMs and waits for the runner to exit; any new
launch path in a test needs the same join, at up to one tick per VM. The
product has the same race: `doStop` sends `finalize` and never waits for the
runner, so `doRelease`'s `RemoveAll` of the state dir can fail the same way.
Kata `v9p5` tracks it, not yet reproduced.

## A darwin stub that always fails trips staticcheck SA4023

On darwin, staticcheck proves that a `//go:build !linux` stub returning an
error built per call always fails, then reports the caller's shared
`err != nil` check as always true. It holds only off Linux, so the Linux lint
pass stays clean while the native one fails; seven findings came from four
stubs. Return a package-level `var errX = errors.New(...)` instead, which
staticcheck cannot prove non-nil, and SA4023 stays on everywhere. Count lint
findings with `--max-same-issues=0 --max-issues-per-linter=0`: the defaults
(3 and 50) drop repeats from the report, and a warm lint cache can replay an
old clean result, so check main with a fresh `GOLANGCI_LINT_CACHE`.

## scripts/linux ships untracked local state

`scripts/linux` filters its rsync through `.gitignore` alone. Directories
hidden in `.git/info/exclude`, `.tracker/` and `.roborev/` here, pass the
filter, so task-runner and review transcripts reach `~/vmobs-build` on the
Linux host with every run; `.dockerignore` does not list `.tracker` either.
Treat anything under them as copied to that host. A test that walks the
